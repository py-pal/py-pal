...
def process():
    ...
    img.save(
        ...
        duration=args.millisecs_per_frame,
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="tool for converting RNG to GIF animation"
    )
    parser.add_argument("rng", type=argparse.FileType("rb"), help="rng file to decode")
    parser.add_argument(
        "-o",
        "--output",
        required=True,
    ...
    parser.add_argument(
        "--algorithm", "-a", default="YJ1", help="decompression algorithm"
    )
    parser.add_argument(
